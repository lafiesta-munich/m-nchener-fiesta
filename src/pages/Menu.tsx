    ...
    <main className="min-h-screen">
      ...
      <section className="section-padding">
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {food.map((category) => (
              <CategoryCard key={category.id} category={category} type="speisen" imageMap={imageMap} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-muted/30">
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {drinks.map((category) => (
              <CategoryCard key={category.id} category={category} type="getraenke" imageMap={imageMap} />
            ))}
          </div>
        </div>
      </section>
...
const CategoryCard = ({ category, type, imageMap }: CategoryCardProps) => {
  const productCount = category.sections.reduce((acc, section) => acc + section.products.length, 0);
